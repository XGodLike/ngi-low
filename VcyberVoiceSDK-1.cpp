...
static void *ThreadPostData(void* parameter)
{
	...
	while (m_thread->b_run)
	{
		...
		{	
			if (g_configs.b_log) 
			{
				p_Timelog->tprintf("[ThreadPostData]!DataBuf.empty()\n");
				pthread_mutex_lock(&seq_lock);
				p_Timelog->tprintf("[ThreadPostData]pthread_mutex_lock\n");
				string data = m_thread->q_buf.front().buf;
				bool last = m_thread->q_buf.front().b_Last;
				m_thread->q_buf.pop();		
				p_Timelog->tprintf("[ThreadPostData]DataBuf.pop()\n");
				pthread_mutex_unlock(&seq_lock);
				p_Timelog->tprintf("[ThreadPostData]pthread_mutex_unlock\n");
...
{	
	...
	if (g_configs.b_dns)
	{
		...
		if(g_configs.m_server_ip == ""){
			ret_code = CLOUDVD_ERR_NO_FIND_SERVER_IP_OR_PORT;
			goto label;
		}
		else{
...
{	
	...
	*handle = sp;
	...
	return (ret_code < 0 ? CLOUDVD_ERR_SYSTEM_FAILED:ret_code);
}
